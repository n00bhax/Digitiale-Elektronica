...
begin
    ...
    --Pauze
       ...
       if (Ball_x-g_BallSize=0) then  
        ...
        elsif (Ball_x+g_BallSize=g_xMax) then  
            Ball_x <= g_xMax/2;
            Ball_y <= g_yMax/2;
...
begin
    if rising_edge(PalletClock) then
        ...
        elsif SW(0)='0' and SW(1)='0' then --CPU vs CPU
             if Ball_Y<PalletL_Y+20 and PalletL_y >=15+PalletL_length then --ervoor zorgen dat hij het niet recht raakt
                              PalletL_Y <= PalletL_Y-1;
             elsif Ball_Y>PalletL_Y-8 and PalletL_y <= g_yMax-PalletL_length-15 then --ervoor zorgen dat hij het niet recht raakt
                              PalletL_Y <= PalletL_Y+1;
             end if;
        else --SW0 staat aan
...
begin
    if rising_edge(PalletClock) then
        if SW(1)='0' then 
            ...
            if Ball_Y<PalletR_Y+15 and PalletR_y >=15+PalletR_length then --ervoor zorgen dat hij het niet recht raakt
                       PalletR_Y <= PalletR_Y-1;
            elsif Ball_Y>PalletR_Y-13 and PalletR_y <= g_yMax-PalletR_length-15 then --ervoor zorgen dat hij het niet recht raakt
                       PalletR_Y <= PalletR_Y+1;
            end if;
